...
export class PointLightNode extends ThreeJSBaseNode {
  constructor() {
    super('Point Light');
    ...
    this.addInputWithLabel('distance', 'Distance', Classic.socket('Number'), 0); // Default distance (0 = infinite)
    this.addInputWithLabel('decay', 'Decay', Classic.socket('Number'), 2); // Default decay rate
    ...
    this.object = new THREE.PointLight(0xffffff, 1, 0, 2); // Initial default light
    this.addThreeJSObjectOutput('light', 'Point Light'); // Output for the THREE.Light object
  }
  ...
  updateObject(inputs, properties) {
    ...
    const distance = inputs['distance']?.length ? inputs['distance'][0] : properties['distance'];
    const decay = inputs['decay']?.length ? inputs['decay'][0] : properties['decay'];
    ...
    if (this.object) {
      if (
        ...
        this.object.distance !== (distance || 0) ||
        this.object.decay !== (decay || 2) ||
        this.object.position.x !== (positionX || 0) ||
        this.object.position.y !== (positionY || 0) ||
      ...
      this.object = new THREE.PointLight(color || 0xffffff, intensity || 1, distance || 0, decay || 2);
      this.object.position.set(positionX || 0, positionY || 0, positionZ || 0);
    }
  }
